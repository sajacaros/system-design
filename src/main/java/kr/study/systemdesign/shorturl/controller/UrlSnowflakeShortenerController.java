...
@RestController
@RequestMapping("/api/url/sf")
public class UrlSnowflakeShortenerController {
    @Autowired
    private ShortUrlProperty property;

    @Autowired
    @Qualifier(value="snowflakeUrlShortenerService")
    ...
    public UrlShortenResponse shortenUrl(@RequestBody @Valid UrlShortenRequest request) {
        ...
        String shortUrl = property.getSnowFlakeShortenerUrlTemplate() + shortKey;
